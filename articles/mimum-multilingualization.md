...
上記のような問題を、出来るだけ簡単に対応していくのが本記事の目的です
...
基本的には、これをマスターファイルのシートとして利用する  
画像のファイルをファイルと同じディレクトリに格納しておき、チップ形式でどこの画像なのかをプレビュー形式で表示できるようにしておく

- ![sheet1_1](../images/mimum-multilingualization/multi-language_gs_sheet1_1.png)

実際に格納しておく画像は、どこの部分のテキストなのかをわかるようにしておく  
...
- 自動で翻訳された英語が、文脈に沿った内容になっているのか、ユビキタス言語に沿った翻訳になっているのかを管理する
  - もし、翻訳された内容が意図しないものになっていれば、自身で編集する
...
例えば、A列の12行目のテキスト「決済」という機会翻訳を使用した場合、表現を使用すると "Settlement" という表現になります。  
ただ、チーム内では "Payment" という表現を一般化しているなどの場合、意味としての理解はできますが、ニュアンスが異なるといった場合があると思います。  
